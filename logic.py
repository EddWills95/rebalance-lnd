...
class Logic:
    ...
    def rebalance(self):
        ...
        if self.first_hop_channel:
            ...
            policy_first_hop = self.lnd.get_policy_to(
                self.first_hop_channel.chan_id)
            min_fee_last_hop = self.compute_fee(
                self.amount, fee_rate_first_hop, policy_first_hop)
        routes = Routes(
            self.lnd,
            payment_request,
        ...
        self.initialize_ignored_channels(
            routes, fee_limit_msat, min_fee_last_hop)

        tried_routes = []
        while routes.has_next():
            route = routes.get_next()

            success = self.try_route(
                payment_request, route, routes, tried_routes)
            if success:
                return True
    ...
    def get_fee_limit_msat(self):
        if self.fee_limit_sat:
            ...
            if self.reckless:
                ...
                fee_limit_msat = max(
                    1_000, self.fee_ppm_limit * self.amount / 1_000)
        elif self.last_hop_channel:
            fee_rate = self.lnd.get_ppm_to(self.last_hop_channel.chan_id)
            if fee_rate > MAX_FEE_RATE:
                last_hop_alias = self.lnd.get_node_alias(
            ...
            fee_limit_msat = self.compute_fee(
                self.amount, self.fee_factor * fee_rate, policy) * 1_000
        ...
        self.output.print_without_linebreak(
            f"Setting fee limit to {fee_limit_formatted} ({ppm_limit_formatted})")
    ...
    def try_route(self, payment_request, route, routes, tried_routes):
        ...
        self.output.print_line("")
        self.output.print_without_linebreak(
            f"Trying route #{len(tried_routes)}")
        fee_msat = route.total_fees_msat
        route_ppm = int(route.total_fees_msat *
                        1_000_000 / route.total_amt_msat)
    ...
    def print_success_statistics(self, route, route_ppm):
        ...
        difference_formatted = format_fee_msat_white(
    ...
    def handle_error(self, response, route, routes):
        ...
        if code == 15:
            ...
            self.output.print_line(format_error(
                f"Unknown error code {repr(code)}:"))
    ...
    def route_is_invalid(self, route, routes):
        ...
        if self.low_outbound_liquidity_after_sending(first_hop, route.total_amt):
            self.output.print_without_linebreak(
                "Outbound channel would have low local ratio after sending, ")
            routes.ignore_first_hop(
                self.get_channel_for_channel_id(first_hop.chan_id))
            return True
        if self.first_hop_and_last_hop_use_same_channel(first_hop, last_hop):
            self.output.print_without_linebreak(
                "Outbound and inbound channel are identical, ")
    ...
    def low_outbound_liquidity_after_sending(self, first_hop, total_amount):
        ...
        if channel is None:
            self.output.print_line(
                f"Unable to get channel information for hop {repr(first_hop)}")
            return True
    ...
    def low_inbound_liquidity_after_receiving(self, last_hop):
        ...
        if channel is None:
            self.output.print_line(
                f"Unable to get channel information for hop {repr(last_hop)}")
            return True
    ...
    def fees_too_high(self, route, routes):
        ...
        if high_fees:
            ...
            transaction_fees_formatted = format_fee_msat(
                int(rebalance_fee_msat), 8)
            missed_fee_formatted = format_fee_msat(
                math.ceil(missed_fee_msat), 8)
    ...
    def initialize_ignored_channels(self, routes, fee_limit_msat, min_fee_last_hop):
        ...
        if self.last_hop_channel and fee_limit_msat and not self.reckless:
            ...
            max_fee_rate_first_hop = math.ceil(
                fee_limit_msat * 1_000 / self.amount)
            for channel in self.lnd.get_channels():
